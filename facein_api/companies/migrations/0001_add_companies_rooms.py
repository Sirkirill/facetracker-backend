# Generated by Django 2.2.12 on 2020-05-24 15:45

import django.db.models.deletion
from django.db import migrations
from django.db import models


class Migration(migrations.Migration):
    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='Company',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False,
                                        verbose_name='ID')),
                ('name',
                 models.CharField(default='FaceIn', max_length=255, verbose_name='Компания')),
                ('is_active', models.BooleanField(default=True,
                                                  help_text='Компания пользуется услугами в данный момент',
                                                  verbose_name='Активная')),
            ],
            options={
                'verbose_name': 'Компания',
                'verbose_name_plural': 'Компании',
            },
        ),
        migrations.CreateModel(
            name='Room',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False,
                                        verbose_name='ID')),
                ('name', models.CharField(max_length=255, verbose_name='Помещение')),
                ('info',
                 models.TextField(blank=True, max_length=1023, verbose_name='Описание комнаты')),
                ('is_whitelisted', models.BooleanField(default=False,
                                                       help_text='В помещение может попасть определенный круг людей (иначе все, кроме черного списка)',
                                                       verbose_name='Помещение с белым списком')),
                ('company', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE,
                                              related_name='rooms', related_query_name='room',
                                              to='companies.Company', verbose_name='Компания')),
            ],
            options={
                'verbose_name': 'Помещение',
                'verbose_name_plural': 'Помещения',
                'unique_together': {('company', 'name')},
            },
        ),
    ]
